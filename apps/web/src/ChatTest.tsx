...
export function ChatTest({ onOpenGap }: { onOpenGap?: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/health', { signal: AbortSignal.any([controller.signal, AbortSignal.timeout(5_000)]) })
      .then(response => setApiAvailable(response.ok))
      .catch(() => { if (!controller.signal.aborted) setApiAvailable(false); });
  ...
  async function send(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(reason instanceof DOMException && reason.name === 'TimeoutError'
        ? 'La evaluación superó el tiempo de espera. Inténtelo de nuevo.'
        : 'No fue posible conectar con la API de NEXA. Verifique que el proyecto esté en ejecución.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return <div className="assistant-layout"><section className="panel conversation"><div className="conversation-head"><div><span className={`badge badge-${apiAvailable === false ? 'red' : apiAvailable ? 'green' : 'neutral'}`}>● {apiAvailable === null ? 'Verificando' : apiAvailable ? 'En línea' : 'Sin conexión'}</span><h2>Asistente de conocimiento</h2></div><span className="muted-cell">{apiAvailable ? 'Conectado a la API de NEXA' : apiAvailable === false ? 'API de NEXA no disponible' : 'Comprobando conexión'}</span></div><div className="messages">
    {!submitted && <div className="assistant-welcome"><span className="nexa-avatar"><Sparkles size={17} /></span><div><h2>¿Qué necesitas saber sobre tu organización?</h2><p>Haz una pregunta sobre sus procesos, políticas o conocimiento aprobado.</p></div></div>}
  ...
  </div><form className="composer" onSubmit={(event) => void send(event)}><label className="sr-only" htmlFor="question">Pregunta</label><input id="question" value={message} onChange={event => setMessage(event.target.value)} placeholder="Escribe una pregunta sobre tu organización…" disabled={loading} /><button aria-label="Enviar pregunta" disabled={loading || !message.trim()}><ArrowRight size={18} /></button></form></section>
  <aside className="context-panel"><h3>Contexto de respuesta</h3><div className="context-section"><span className="context-label">Estado</span><p>{result ? sufficient ? 'La respuesta usa conocimiento suficiente y trazable.' : result.organizationallyRelevant ? 'La consulta reveló conocimiento que debe recuperarse.' : 'La consulta no corresponde al conocimiento organizacional disponible.' : 'Envía una pregunta para evaluar el conocimiento disponible.'}</p></div><div className="context-section"><span className="context-label">Alcance</span><p>NEXA responde con conocimiento organizacional disponible y señala cuando falta información validada.</p></div></aside></div>;
}
