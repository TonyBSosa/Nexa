...
| ContactPerson | Synthetic directory entry: `id`, `name`, `title`, `department`, `email`, `phone`: string/null, `availability` (`AVAILABLE`/`BUSY`/`AWAY`/`UNKNOWN`). Tentative role mapping from department/experts; not verified identity. |
