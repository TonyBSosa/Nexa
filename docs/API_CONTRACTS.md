...
- `POST /api/knowledge-gaps/:id/evidence-items`: 201. `{type, source, author, evidenceDate, reference?, note?, content?, url?, file?, meeting?}`. `type` is one of MANUAL_TEXT, MEETING_NOTES, TRANSCRIPT, PDF, DOCUMENT, IMAGE, LINK, VIDEO, VIDEO_LINK, SNIPPET, and determines which content field applies; the rest are rejected. `evidenceDate` is YYYY-MM-DD and cannot be in the future. Files validate MIME type against extension and a per-type size limit (PDF and DOCUMENT 10 MB, IMAGE 5 MB, VIDEO 25 MB); file bytes are not stored, only metadata. Requires IN_PROGRESS or KNOWLEDGE_COLLECTED.
...
Two existing endpoints gained conditional behaviour so current flows keep working. `POST /api/knowledge-gaps/:id/transition` to AWAITING_APPROVAL additionally requires a complete checklist, but only for gaps whose draft review has already been started. `POST /api/knowledge-gaps/:id/approval` accepts an optional `actor` and, when reviewers have been assigned, applies the reviewer rules: the author cannot decide and only an assigned reviewer can. The recorded actor is the normalized name. Gaps that never use the panel keep the previous unattributed behaviour.
