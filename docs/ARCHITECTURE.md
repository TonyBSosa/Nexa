...
Prefer one structured assessment operation over separate calls for classification and suggestions. The Botpress adapter may repeat that complete operation once to recover a failure or confirm relevant insufficiency. Botpress-specific payloads stay inside the adapter. FAILURE never creates a gap. A non-relevant result cannot authorize gap creation regardless of sufficiency status. Exact shared conceptual result fields are in `API_CONTRACTS.md`.
...
The Botpress adapter uses the official TypeScript client and Runtime API rather than the legacy Chat integration. A bot-context client discovers and caches an active integration/channel and its installed integration ID for the API process; an integration-context client then creates an isolated user and conversation for each assessment. Polling ends on a valid envelope, the overall timeout, or a bounded quiet period after non-structured output. No participant call is required for this documented message flow. `x-integration-alias` is not used: `integrationId` establishes the required Runtime integration context. BotpressAgentProvider sends `NEXA_ASSESSMENT_V1`, a blank line, and the original question; it extracts and validates only `nexa.assessment.v1` JSON, including an envelope wrapped in Markdown or conversational text. A failure receives at most one fresh retry. A relevant INSUFFICIENT assessment receives one fresh confirmation; SUFFICIENT wins, while two valid relevant INSUFFICIENT results are required for gap eligibility. All Botpress types and safe authentication/authorization/timeout classifications stay inside the adapter. Draft generation uses the shared deterministic evidence-only helper and never selects FakeAgentProvider implicitly.
...
Retain the eight stages without requiring a screen per state. Detection acceptance uses the dedicated review decision endpoint and enters TRIAGED; a human may return TRIAGED to DETECTED through that endpoint. Classification confirmation records the declared reviewer and timestamp, requires category, department, responsible person and importance, and leaves the gap in TRIAGED ready for ACTION_PROPOSED. The generic transition endpoint cannot bypass these gates. Metadata edits invalidate classification confirmation. Evidence addition and draft generation/save do not advance lifecycle status. Every evidence addition increments gap evidenceRevision. A draft records its input evidenceRevision; if evidence changes, the old draft is stale. Require a new draft revision before review/approval. Evidence is editable only in the collection states defined by the API, not while under review. CHANGES_REQUESTED and REJECTED return to KNOWLEDGE_COLLECTED and prohibit unchanged resubmission: a newer draft revision is mandatory. Rejection rejects the draft, not the gap.
...
| Insufficient evidence | Confirm once with a fresh assessment; persist/link the relevant gap only when both valid results are relevant and insufficient. |
| Malformed external response | Retry once, then persist a controlled FAILURE attempt without creating a gap. |
