...
## ADR-027 — Human detection review and classification gate

**Decision:** Extend DETECTED and TRIAGED with separate review dispositions, reversible discard/duplicate decisions, declared reviewers, classification assignment and revision-guarded audit history. Preserve all eight lifecycle stages. Acceptance is DETECTED → TRIAGED; an explicit review return allows TRIAGED → DETECTED. Confirmation stays TRIAGED and enables the existing ACTION_PROPOSED transition. This supersedes the strictly forward-only early-stage flow and narrow metadata scope of ADR-021/023 for these two stages only.

**Consequences:** Add review and decision tables to the unified SQLite schema v5. No physical deletion, inferred identity, authentication, autonomous action or provider changes. Require reasons for discard, category/department/responsible person/importance for classification, and reject stale decisions. Metadata edits invalidate classification. Duplicate marking and related knowledge are references, not merges or publication. Existing occurrence counting and historical metrics are retained, including requests under discarded/duplicate dispositions. Review list pagination/filtering is a new endpoint, preserving the original list contract. Keyword warnings and lexical similarity are advisory. Update the demo to perform explicit review and classification before recovery.
...
## ADR-027 — Evidence versioning and draft review as a separate stage

**Decision:** Keep the human review of the request (triage) and the review of the draft as two separate stages with separate storage and separate routes. Triage keeps `gap_reviews`, `gap_review_events` and `/knowledge-gaps/:id/review`; draft review gets `draft_reviews`, `draft_review_comments` and `/knowledge-gaps/:id/draft-review`. Extend `collected_evidence` in place with type, authorship, file and meeting metadata and per-item versions, keeping `sourceType = 'MANUAL'`.

**Rationale:** Both stages were called review and collided on the name `ReviewDecision` and on the route prefix. They answer different questions — whether the request should be handled at all, and whether the written answer is correct — and have different actors. Extending the evidence table avoids a twelve-step rebuild and keeps `CollectedEvidence`, the draft freshness rule and the existing tests intact, because all evidence is still entered by a person.

**Consequences:** Schema version 6 adds columns only when missing, so a database can reach the upgrade block already carrying some of them. Any evidence change advances `evidenceRevision` and therefore invalidates the draft that used it, which is the intended behaviour for withdrawal and replacement. File bytes are not stored; only metadata is recorded, and the view and download URLs are placeholders. The checklist gate and the reviewer rules are conditional on the panel having been used, so gaps created before this stage keep their previous behaviour; making them unconditional is pending and will require updating the older tests. Actor remains declared text without verified identity.
