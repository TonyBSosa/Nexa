...
**Consequences:** Schema version 6 adds columns only when missing, so a database can reach the upgrade block already carrying some of them. Any evidence change advances `evidenceRevision` and therefore invalidates the draft that used it, which is the intended behaviour for withdrawal and replacement. File bytes are stored on disk beside the database and served back under a restrictive content security policy, while the database keeps the metadata. The checklist gate and the author rule apply to every gap without exception, which required attributing every approval: an anonymous decision cannot be checked against the author. Actor remains declared text without verified identity.
