...
Detection/classification extension: before recovery, enter a declared reviewer to accept DETECTED into TRIAGED. Save category, department, responsible person and importance, inspect the summary and confirm classification before advancing to ACTION_PROPOSED. Demonstrate discard/restore and duplicate marking separately so the primary scenario counts remain reproducible. Generic DETECTED → TRIAGED transitions are now blocked in favor of the explicit review decision. Use synthetic names and content only; sensitivity warnings are advisory.

The implemented fake-provider rehearsal uses the synthetic printer-retirement question documented in README.md; the Finance story above remains the planned presentation scenario. Accept and classify through the review endpoints, then ACTION_PROPOSED, approve/select the simulated action to enter IN_PROGRESS, optionally return to Acción propuesta with a justification, add evidence, explicitly confirm KNOWLEDGE_COLLECTED, generate/save a draft, and explicitly submit AWAITING_APPROVAL. Human approval publishes; a separate closure resolves. Repeat the printer question before and after closure to verify approved article evidence, unchanged occurrences, and no new gap. No Botpress or external execution is required for this development rehearsal.
