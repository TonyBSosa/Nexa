...
| FR-006 | Match open gaps using deterministic normalizedQuestionKey: lowercase, trim, replace punctuation with spaces, collapse whitespace, and apply supported demo aliases. Keep AI display titles separate. Persist/link each accepted unanswered submission transactionally; new gaps start at 1. With optional clientSessionId, suppress repeat demand increments for five minutes per session/gap; otherwise increment once. No embeddings or confidence thresholds for matching. |
